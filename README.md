...
3. **Alignment**: A Multiple Sequence Alignment (MSA) file (e.g., `.fa` or `.aln`) of your structures, generated by tools like [PROMALS3D](https://bio.tools/promals3d).
...
```bash
python scripts/generate_multi.py \
    input_pdbs.txt \
...
<!-- 1. Generate a "multinetwork" consisting of aligned RINs for all structures:
...
A. Normalize and sum the individual networks from the multinetwork to generate a sum network:
```bash
python scripts/analysis_sum.py \
...
B. Perform additional network analyses as desired; see the [examples](examples/) directory.
